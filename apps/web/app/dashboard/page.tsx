 ...
 * 上から: 今やること → 今月の数字 → 今月の目標 / マイルストーン → 予定 / レビュー → 設定。
 * 文言は lib/dashboard/messages.ts（マスタ＋表示ロジック）、指標は metrics.ts、達成は milestones.ts。
 *
 * 2026-09: 左ナビ（DashboardNav）は廃止。同じリンクが右上の ☰（components/SideMenu）に
 * あって二重だったうえ、スマホでは本文の前に長いリンク列が居座って読み始められなかった。
 * ナビにしか無かった設定への導線は、末尾の「設定」カード（状態つき）で受けている。
 */
...
export default async function DashboardPage() {
  ...
  const avatarUrl = profileRow?.avatarUrl ?? me.avatarUrl ?? null;
  ...
  return (
    <main className="bg-background text-foreground">
      ...
      {/* 左ナビを外したぶん本文が広がりすぎないよう、以前の右カラム幅（約 950px）に合わせる */}
      <div className="mx-auto max-w-[980px] px-5 py-8 sm:px-10 sm:py-10">
        {/* ===== ヘッダー ===== */}
        {/* flex-1 は flex-basis 0 なので、min-w を与えないと右側に押されて
            ...
            アバター（44px + gap 12px）を足したぶん、320px で右へはみ出さないよう
            スマホの下限は 15rem → 12rem に下げる（248px なら 320px 幅に収まる）。 */}
        <header className="flex flex-wrap items-start gap-3">
          {/* 左ナビにしか無かった「誰としてログインしているか」（アバター・学校）を見出しに移した */}
          <div className="flex flex-1 items-start gap-3 max-sm:min-w-[12rem]">
            <div className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-xl bg-neutral-900 text-[15px] font-bold text-primary-500">
              {avatarUrl ? (
        ...
        {/* ===== 5. 設定 =====
            左ナビを廃止したので、そこにしか無かった設定への導線をここで受ける。
            ただのリンク列にせず、今の状態（メニューの有無・空き枠・完成度など）を
            一緒に出して、上のカードと同じ「見て → 飛ぶ」流れに揃えた。 */}
        <Section title="設定" hint="出品と見せ方の設定はここから">
          <div className="grid grid-cols-2 gap-2.5 lg:grid-cols-3">
            <SettingCard href="/settings/services" label="相談メニュー・プラン" hint={hasMenu ? '料金と時間を見直す' : 'まだありません'} />
            <SettingCard href="/settings/availability" label="空き時間" hint={`今週の空き ${m.todo.openSlots7d} 枠`} />
            <SettingCard href="/settings/profile" label="プロフィール・発信" hint={`完成度 ${completeness.percent}%`} />
            {/* hint は学歴（users.education の自己申告）ではなく在籍確認の申請状況を出す。
                学歴を出すと、確認を一度も出していない人にも「○○大学 ・ 在学中」と表示されて
                確認済みに読めてしまい、このカードが唯一の導線なのに未申請の人が素通りする。 */}
            <SettingCard href="/settings/verification" label="在籍確認・資格" hint={completeness.recommended.verification ? '申請ずみ' : '未申請（学生証を提出）'} />
            <SettingCard href="/settings" label="公開ステータス" hint={m.profilePublished ? '公開中' : '非公開'} />
            <SettingCard href="/settings/account" label="通知・アカウント" hint="通知メールとログイン情報" />
          </div>
        </Section>
      </div>
    </main>
...
/** 設定ページへのカード。左ナビの代わりなので、リンク名だけでなく今の状態も 1 行出す */
function SettingCard({ href, label, hint }: { href: string; label: string; hint: string }) {
  return (
    <Link href={href} className="group flex items-start gap-2 rounded-xl border border-border bg-card px-3.5 py-3 transition hover:border-foreground">
      {/* 縮むのはラベル側。矢印は shrink-0 で、日本語ラベルが 1 文字ずつ潰れないようにする */}
      <span className="min-w-0 flex-1">
        <span className="block text-[12.5px] font-semibold">{label}</span>
        {/* 320px では 2 列でラベル領域が約 85px しかなく、可変長の状態文字列が伸びると
            グリッド行の高さが隣のカードに引きずられて不揃いになる。2 行で頭打ちにする */}
        <span className="mt-0.5 line-clamp-2 block text-[11px] leading-[1.5] text-neutral-500">{hint}</span>
      </span>
      <ArrowUpRight className="mt-0.5 h-3.5 w-3.5 shrink-0 text-neutral-300 transition group-hover:text-neutral-500" aria-hidden />
    </Link>
  );
}
