...
/**
 * 居住確認の申請フォーム (Client Component)。
 *
 * - 書類は最大 3 枚まで。Storage に逐次アップロードして path 配列を作る
 * - country は ISO 2 文字、city は自由入力
 * - documentType は visa / residence_card / utility_bill / tax_certificate / other
 * - userNote は任意の補足
 *
 * 送信時:
...
const DOC_TYPES = [
  { value: 'utility_bill', label: '光熱費 (EDF / Engie / 水道など)' },
  { value: 'residence_card', label: '在留カード / Titre de séjour' },
  { value: 'visa', label: 'ビザ (滞在許可)' },
  { value: 'tax_certificate', label: '住民税・所得税の証明' },
  { value: 'other', label: 'その他 (賃貸契約書など)' },
] as const;
    ...
    >
      ...
      <div>
        ...
        <p className="mt-1 text-[11px] text-foreground/55">
          氏名と住所が読める書類をご提出ください。給与額や口座番号など、確認に
          不要な部分は黒塗りでマスクして頂いて構いません。
        </p>
      </div>
      ...
      <div>
        <label className="mb-2 block text-[12px] font-medium text-foreground/70">
          書類ファイル (1〜3 枚、各 15MB まで、JPEG/PNG/HEIC/PDF)
        </label>
        <input
          ref={fileInputRef}
          type="file"
