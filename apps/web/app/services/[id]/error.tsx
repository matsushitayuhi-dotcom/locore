...
/**
 * /services/[id] 専用エラーバウンダリ。
 *
 * Server Component で例外が出たとき、Vercel のデフォルト白画面ではなく
 * エラー詳細を **画面に表示** することで、Vercel ログにアクセスせずに
 * 原因を特定する。本番デバッグ用、安定したら情報量は減らす。
 */
export default function ServiceDetailError({
  ...
  useEffect(() => {
    console.error('[services/[id] error.tsx]', error);
  }, [error]);
