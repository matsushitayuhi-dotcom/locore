...
### ユーザー視点の入口は **2択**（シンプルさ優先）

書き手がエディタで最初に選ぶのは2つだけ:

| 入口（書き手が選ぶ） | 内部 type | 説明 |
|---|---|---|
| **モデルコース** | `itinerary` | 順序のある体験。スポットに順番・時間・移動が付く |
| **ブログ** | `standard` | 順序のない記事。場所紹介でも読み物でもOK |

### ブログ（standard）の表示は **スポット情報の有無で自動判定**

「ブログ」を選んだ記事は、**場所（スポット）ブロックが1つでもあるか**で出力レイアウトを自動で振り分ける。書き手は意識しなくてよい:

| 条件 | 自動レイアウト | 中身 |
|---|---|---|
| 場所ブロックあり | **場所紹介（place-guide）** | 場所を順不同で紹介＋末尾にピン地図＋スポット一覧 |
| 場所ブロックなし | **読み物（essay）** | 文章・写真・動画主体。地図なし |

| type | 場所ブロック | 地図 | 順番/時間/移動 |
|---|---|---|---|
| `itinerary`（モデルコース） | あり | あり（順路） | **あり** |
| `standard`（ブログ） | 任意（あれば place-guide、なければ essay） | あれば（ピン） | なし |

→ **内部 type は `itinerary | standard` の2分岐**。ブログの place-guide / essay は
「場所ブロックがあるか」だけで自動分岐し、**入口・エディタ・データモデルは2タイプで統一**する。
