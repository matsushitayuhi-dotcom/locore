...
> 旅行記事は「各 駐在員 の PR・実績ポートフォリオ」であり、同時に「単体として売れる商品」でもある、というデュアル位置付けを核に据える。

### 13.1 双方向の発見フロー

**A 流れ — 興味駆動 / PR funnel**
```
記事を読む → 「この人面白い」→ 駐在員 プロフィール → サービス → 予約
```
...
### 13.2 二層モデル: 駐在員 と 旅行者

**Locore は終始 2 つのロールしか持たない**:

| ロール | 役割 | 既存 cookie mode |
|---|---|---|
| **駐在員** (Resident) | 提供側。記事を書く・サービスを出品する・取引を受ける | `resident` |
| **旅行者** (Traveler / 利用者) | 消費側。記事を読む・サービスを予約する | `traveler` |

「Creator」「Seller」「Writer」を独立した抽象として導入しない。**駐在員 = 記事プロバイダー & サービスプロバイダー** の合体ロールで、`writer_profiles` と `seller_profiles` は **同じ駐在員に対して任意で生える** 2 つの能力プロフィール:

```
users (id) — 全員
  │
  ├─ writer_profiles (user_id) — 記事を書く能力 (任意)
  │
  └─ seller_profiles (user_id) — サービスを売る能力 (任意)
        │
...
駐在員の中には:
- **記事だけ** 書く人 (writer_profiles のみ)
- **サービスだけ** 出す人 (seller_profiles のみ)
- **両方** やる人 (両方持つ) ← Locore の理想形

旅行者側には profile 拡張テーブルは無い。`users` 行のみ。

#### 駐在員プロフィール (`/residents/[handle]`) = ハブ

すべての 駐在員 が `/residents/[handle]` という単一の正規 URL を持つ。記事と Listing は両方ともこのハブから生える:
...
#### `/residents/[handle]` のタブ構成
- **概要**: bio・在住年数・residency_verifications バッジ・KYC バッジ・取り扱いカテゴリ
- **記事**: 公開済み articles 一覧（ローカル度バッジ、有料/無料区別） ※ writer_profiles を持つ場合のみ
- **出品**: published listings 一覧（KYC 確認済みのみ取引可、price range 表示） ※ seller_profiles を持つ場合のみ
- **レビュー**: 記事レビュー (`reviews`) + 取引レビュー (`order_reviews`) を別タブで統合表示
- **問い合わせ**: チャット起点（取引前は user_services 経由、取引後は order_messages）

旧来の `/writers/[id]` と `/residents/[id]` は `/residents/[handle]` に **301 リダイレクト**。「Writer」「Creator」「Seller」というラベルは UI から完全撤去し、すべて **駐在員** に統一する。
...
│ Marais 古着同行       │
│ 高橋まりか (パリ7年)  │
│ ★4.8 (24件) 記事3本  │ ← ここが 駐在員-centric の核
│ ✓ 在住確認 ✓ KYC      │
└──────────────────────┘
```
...
6. **よく似たサービス** (同 category × 他 駐在員) — 比較検討用
...
| この 駐在員 の他の Listings | seller の published listings 3 件 | `WHERE seller_id = article.writer_id ORDER BY popularity` |
...
**核心**: 駐在員 の片方の活動 (記事 or Listing) が反対側の信頼を底上げする。記事が増えるほどサービスが売りやすくなり、取引が増えるほど記事の信頼度が上がる **二方向のフライホイール**。
...
[検索: "ワインアテンド"]
┌───────────┬──────────┬──────────┐
│ 駐在員s  │ Services │ Articles │
│  3 件     │  4 件    │  12 件   │
└───────────┴──────────┴──────────┘
```

- **駐在員s タブ** (新規・最重要): キーワードに該当する記事 X 本 + Listing Y 件を持つ 駐在員 を集約表示。**プロフィール直行が最短導線**。
...
### 13.8 オンボーディング (駐在員 新規登録時)

1. アカウント作成 + residency_verifications 申請
2. **最初の記事を 1 本書く** (PR の核、必須)
3. 任意で Listing 作成 → seller_profiles + Stripe Connect Express
4. Listing 作成時に「関連記事を 1 つ選ぶ」ステップで自然に 駐在員-link が貼られる

これにより「記事だけ書いて消える人」を最小化し、駐在員 として継続活動する設計圧をかける。

### 13.9 分析ダッシュボード (駐在員 視点)

`/dashboard` または `/dashboard/insights`:
...
| `/writers/[id]` `/residents/[id]` 散在 + Creator 概念導入の検討 | `/residents/[handle]` 単一ハブ、Creator 抽象は廃案。駐在員 = 提供者で完結 |
| `/search` 2 軸 (articles + listings) | `/search` 3 軸 (駐在員s + Services + Articles) |
...
| **`/services/[id]`** | サービス詳細 (駐在員 セクションに記事カルーセル埋め込み) | B 流れの転換点 |
| **`/residents/[handle]`** | 駐在員プロフィール ハブ (概要・記事・出品・レビュー・問い合わせ) | A / B 両方の合流点 |
...
| `/dashboard/insights` | 駐在員 分析 (記事→Listing 遷移率等、§13.9) | |
...
#### 検索 / ホーム
- `/search`: 2 軸 → **3 軸** (駐在員s / Services / Articles) に拡張 (§13.6)
...
#### 駐在員 ハブへの統合
- `/writers/[id]` → `/residents/[handle]` に 301 redirect
- `/residents/[id]` → `/residents/[handle]` に 301 redirect
- (handle 未設定の旧ユーザーは uuid → /residents/[uuid] のまま許容、UI で handle 設定を促進)
...
§13 の 駐在員-centric 設計に伴い、`articles.related_listing_id` (1:0..1) を廃止して N:N 中間テーブルに格上げする:
...
`/residents/[handle]` URL のために `users.handle text UNIQUE` を追加:
...
移行:
- 既存ユーザーには handle 未設定で /residents/<uuid> URL を一時的に許容
